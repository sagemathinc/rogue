...
void doadd(char *fmt, va_list args) {
  ...
  if (strlen(buf) + newpos >= MAXMSG) endmsg();
  strcat(msgbuf, buf);
...
void status() {
  ...
  if (s_hp == pstats.s_hpt && s_exp == pstats.s_exp && s_pur == purse &&
      ...
      s_hungry == hungry_state && !stat_msg)
    return;
  ...
  if (s_hp != max_hp) {
    temp = max_hp;
    s_hp = max_hp;
    for (hpwidth = 0; temp; hpwidth++) temp /= 10;
  }
  ...
  if (stat_msg) {
    move(0, 0);
    msg("Level: %d  Gold: %-5d  Hp: %*d(%*d)  Str: %2d(%d)  Arm: %-2d  Exp: "
        "%d/%ld %c%c %s",
        level, purse, hpwidth, pstats.s_hpt, hpwidth, max_hp, pstats.s_str,
        max_stats.s_str, 10 - s_arm, pstats.s_lvl, pstats.s_exp,
        (rookie_mode ? 'r' : 'w'), rogue_version, state_name[hungry_state]);
  } else {
    move(STATLINE, 0);

    printw(
        "Level: %d  Gold: %-5d  Hp: %*d(%*d)  Str: %2d(%d)  Arm: %-2d  Exp: "
        "%d/%d %c%c %s",
        level, purse, hpwidth, pstats.s_hpt, hpwidth, max_hp, pstats.s_str,
        max_stats.s_str, 10 - s_arm, pstats.s_lvl, pstats.s_exp,
        (rookie_mode ? 'r' : 'w'), rogue_version, state_name[hungry_state]);
