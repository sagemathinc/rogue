...
void look(bool wakeup) {
  ...
  for (y = sy; y <= ey; y++)
    if (y > 0 && y < NUMLINES - 1)
      for (x = sx; x <= ex; x++) {
        ...
        fp = &pp->p_flags;
        if (pch != DOOR && ch != DOOR)
          if ((pfl & F_PASS) != (*fp & F_PASS)) continue;
        if (((*fp & F_PASS) || ch == DOOR) && ((pfl & F_PASS) || pch == DOOR)) {
          if (hero.x != x && hero.y != y && !step_ok(chat(y, hero.x)) &&
              !step_ok(chat(hero.y, x)))
            continue;
        }
        ...
        if (on(player, ISBLIND) && (y != hero.y || x != hero.x)) continue;

        move(y, x);

        if ((proom->r_flags & ISDARK) && !see_floor && ch == FLOOR) ch = ' ';

        if (tp != NULL || ch != CCHAR(inch())) addch(ch);

        if (door_stop && !firstmove && running) {
          switch (runch) {
            case 'h':
              if (x == ex) continue;
              when 'j' : if (y == sy) continue;
              when 'k' : if (y == ey) continue;
              when 'l' : if (x == sx) continue;
              when 'y' : if ((y + x) - sumhero >= 1) continue;
              when 'u' : if ((y - x) - diffhero >= 1) continue;
              when 'n' : if ((y + x) - sumhero <= -1) continue;
              when 'b' : if ((y - x) - diffhero <= -1) continue;
          }
          switch (ch) {
            case DOOR:
              if (x == hero.x || y == hero.y) running = FALSE;
              break;
            case PASSAGE:
              if (x == hero.x || y == hero.y) passcount++;
              break;
            case FLOOR:
            case '|':
            case '-':
  ...
  if (!running || !jump) mvaddch(hero.y, hero.x, PLAYER);
#ifdef DEBUG
  done = FALSE;
#endif /* DEBUG */
}
...
int trip_ch(int y, int x, int ch) {
  if (on(player, ISHALU) && after) switch (ch) {
      case FLOOR:
      case ' ':
      case PASSAGE:
      case '-':
...
THING *find_obj(int y, int x) {
  ...
  for (obj = lvl_obj; obj != NULL; obj = next(obj)) {
    if (obj->o_pos.y == y && obj->o_pos.x == x) return obj;
  }
#ifdef MASTER
...
void chg_str(int amt) {
  ...
  comp = pstats.s_str;
  if (ISRING(LEFT, R_ADDSTR)) add_str(&comp, -cur_ring[LEFT]->o_arm);
  if (ISRING(RIGHT, R_ADDSTR)) add_str(&comp, -cur_ring[RIGHT]->o_arm);
...
void aggravate() {
  ...
  for (mp = mlist; mp != NULL; mp = next(mp)) runto(&mp->t_pos);
}
...
bool is_current(THING *obj) {
  ...
  if (obj == cur_armor || obj == cur_weapon || obj == cur_ring[LEFT] ||
      obj == cur_ring[RIGHT]) {
    if (!terse) addmsg("That's already ");
    msg("in use");
    return TRUE;
  }
  return FALSE;
...
bool get_dir() {
  ...
  if (again && last_dir != '\0') {
    ...
    until(gotit);
    if (isupper(dir_ch)) dir_ch = (char)tolower(dir_ch);
    last_dir = dir_ch;
    last_delt.y = delta.y;
    last_delt.x = delta.x;
  }
 ...
 */
int spread(int nm) { return nm - nm / 20 + rnd(nm / 10); }
...
void call_it(struct obj_info *info) {
  if (info->oi_know) {
    ...
    if (get_str(prbuf, stdscr) == NORM) {
      if (info->oi_guess != NULL) memfree(info->oi_guess);
...
char rnd_thing() {
  int i;
  static char thing_list[] = {POTION, SCROLL, RING,   STICK, FOOD,
                              WEAPON, ARMOR,  STAIRS, GOLD,  AMULET};
 ...
 *	player is tripping
 */
char *choose_str(char *ts, char *ns) { return (on(player, ISHALU) ? ts : ns); }
