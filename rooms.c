...
void do_rooms() {
   ...
   */
  left_out = rnd(4);
  for (i = 0; i < left_out; i++) rooms[rnd_room()].r_flags |= ISGONE;
  /*
  ...
  for (i = 0, rp = rooms; i < MAXROOMS; rp++, i++) {
    ...
    if (rnd(10) < level - 1) {
      ...
      if (rnd(15) == 0) rp->r_flags = ISMAZE; /* maze room */
    }
    /*
    ...
    if (rp->r_flags & ISMAZE) {
      rp->r_max.x = bsze.x - 1;
      rp->r_max.y = bsze.y - 1;
      if ((rp->r_pos.x = top.x) == 1) rp->r_pos.x = 0;
...
void dig(int y, int x) {
  ...
  for (;;) {
    ...
    for (cp = del; cp <= &del[3]; cp++) {
      newy = y + cp->y;
      newx = x + cp->x;
      if (newy < 0 || newy > Maxy || newx < 0 || newx > Maxx) continue;
      if (flat(newy + Starty, newx + Startx) & F_PASS) continue;
      if (rnd(++cnt) == 0) {
        nexty = newy;
        nextx = newx;
      }
 ...
 */
bool find_floor(struct room *rp, coord *cp, int limit, bool monst, int fflags) {
  PLACE *pp;
  int cnt;
  char compchar = 0;
  bool pickroom;
  ...
  if (!pickroom) compchar = ((rp->r_flags & ISMAZE) ? PASSAGE : FLOOR);
  cnt = limit;
  for (;;) {
    if (limit != NOLIMIT && cnt-- == 0) return FALSE;
    if (pickroom) {
      rp = &rooms[rnd_room()];
    ...
    if (monst) {
      if (pp->p_monst == NULL && step_ok(pp->p_ch)) return TRUE;
