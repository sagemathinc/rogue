...
void new_level() {
   ...
   * setup t_room ptr
   */
  for (tp = mlist; tp != NULL; tp = next(tp)) tp->t_room = roomin(&tp->t_pos);
  ...
  if (rnd(10) < level) {
    ...
    while (i--) {
       ...
       */
      do {
        if (!find_floor((struct room *)NULL, &stairs, MAXTRIES, FALSE,
                        FF_NOHERO))
          goto skip;
...
void put_things() {
   ...
   */
  if (rnd(rookie_mode ? TREAS_ROOM_ROOKIE : TREAS_ROOM) == 0) treas_room();
