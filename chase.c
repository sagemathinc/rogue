...
void runners() {
  ...
  for (tp = mlist; tp != NULL; tp = next) {
    ...
    if (!on(*tp, ISHELD) && on(*tp, ISRUN)) {
      ...
      if (move_monst(tp) == -1) continue;
      if (on(*tp, ISFLY) && dist_cp(&hero, &tp->t_pos) >= 3) move_monst(tp);
...
over:
  ...
  if (!chase(th, &this)) {
    if (ce(this, hero)) {
      ...
      if (th->t_type != 'F') stoprun = TRUE;
    }
  } else {
    if (th->t_type == 'F') return (0);
  }
  relocate(th, &ch_ret);
  /*
   * And stop running if need be
   */
  if (stoprun && ce(th->t_pos, *(th->t_dest))) th->t_flags &= ~ISRUN;
  return (0);
}
...
bool see_monst(THING *mp) {
  ...
  if (dist(y, x, hero.y, hero.x) < LAMPDIST) {
    if (y != hero.y && x != hero.x && !step_ok(chat(y, hero.x)) &&
        !step_ok(chat(hero.y, x)))
      return FALSE;
    return TRUE;
  }
  if (mp->t_room != proom) return FALSE;
...
bool chase(THING *tp, coord *ee) {
   ...
   */
    ...
    ey = er->y + 1;
    if (ey >= NUMLINES - 1) ey = NUMLINES - 2;
    ex = er->x + 1;
    if (ex >= NUMCOLS) ex = NUMCOLS - 1;
    ...
    for (x = er->x - 1; x <= ex; x++) {
      ...
      for (y = er->y - 1; y <= ey; y++) {
        ...
        if (step_ok(ch)) {
           ...
           */
          if ((obj = moat(y, x)) != NULL && obj->t_type == 'X') continue;
          /*
 ...
 */
int dist_cp(coord *c1, coord *c2) { return dist(c1->y, c1->x, c2->y, c2->x); }
