  ...
  switch (ch) {
    ...
    default:
      ...
      move_stuff:
        ...
        if ((fl & F_PASS) && chat(oldpos.y, oldpos.x) == DOOR) leave_room(&nh);
        hero = nh;
      }
  }
}
  ...
  if (!(rp->r_flags & ISGONE))
    for (y = rp->r_pos.y; y < rp->r_pos.y + rp->r_max.y; y++)
      for (x = rp->r_pos.x; x < rp->r_pos.x + rp->r_max.x; x++)
        if (isupper(winat(y, x))) wake_monster(y, x);
}

bool is_trap(coord *tc) {
  char fl = flat(tc->y, tc->x);
  char ch = winat(tc->y, tc->x);
  if (!(fl & F_REAL) && ch == FLOOR) return TRUE;
  return FALSE;
}
...
char be_trapped(coord *tc) {
  ...
  switch (tr) {
    case T_DOOR:
      ...
      when T_BEAR : no_move += BEARTIME;
      msg("you are caught in a bear trap");
      when T_MYST : switch (rnd(11)) {
        case 0:
      ...
      when T_SLEEP : no_command += SLEEPTIME;
      player.t_flags &= ~ISRUN;
      msg("a strange white mist envelops you and you fall asleep");
      when T_ARROW : if (swing(pstats.s_lvl - 1, pstats.s_arm, 1)) {
      ...
      when T_DART : if (!swing(pstats.s_lvl + 1, pstats.s_arm, 1))
      ...
      when T_RUST : msg("a gush of water hits you on the head");
...
coord *rndmove(THING *who) {
   ...
   */
  if (y == who->t_pos.y && x == who->t_pos.x) return &ret;
