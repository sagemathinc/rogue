  ...
  switch (obj->o_which) {
    case WS_LIGHT:
      ...
      if (proom->r_flags & ISGONE)
        ...
        if (!terse) addmsg(" by a shimmering %s light", pick_color("blue"));
        endmsg();
      }
      when WS_DRAIN :
          /*
      ...
      when WS_SLOT_2 : if (ISVERSION_5_3()) {
        delta.y += hero.y;
        delta.x += hero.x;
      ...
      when WS_POLYMORPH : case WS_TELAWAY : case WS_TELTO : case WS_CANCEL :
          /* case WS_INVIS_5_4: */
          ws_invis_5_4_label : y = hero.y;
      x = hero.x;
      ...
      if ((tp = moat(y, x)) != NULL) {
        ...
        switch (obj->o_which) {
          case WS_INVIS_5_4:
            tp->t_flags |= ISINVIS;
            if (cansee(y, x)) mvaddch(y, x, tp->t_oldch);
            break;
          case WS_POLYMORPH: {
            THING *pp;
          ...
          case WS_CANCEL:
            ...
            if (see_monst(tp)) mvaddch(y, x, tp->t_disguise);
            break;
          case WS_TELAWAY:
          case WS_TELTO: {
            coord new_pos;
            ...
            if (obj->o_which == WS_TELAWAY) {
              do {
                if (!find_floor(NULL, &new_pos, MAXTRIES, TRUE, 0x0)) goto skip;
      ...
      when WS_MISSILE : ws_info[WS_MISSILE].oi_know = TRUE;
      bolt.o_type = '*';
      ...
      when WS_HASTE_M : case WS_SLOW_M : y = hero.y;
      x = hero.x;
      while (step_ok(winat(y, x))) {
        y += delta.y;
        x += delta.x;
      ...
      when WS_ELECT : case WS_FIRE : case WS_COLD
          : if (obj->o_which == WS_ELECT) name = "bolt";
      else if (obj->o_which == WS_FIRE) name = "flame";
      else name = "ice";
...
void fire_bolt(coord *start, coord *dir, char *name) {
        ...
        if (!hit_hero && (tp = moat(pos.y, pos.x)) != NULL) {
          ...
          if (!save_throw(VS_MAGIC, tp)) {
            ...
            if (tp->t_type == 'D' && strcmp(name, "flame") == 0) {
              ...
              if (!terse) addmsg(" off the dragon");
              endmsg();
            } else
              hit_monster(unc(pos), &bolt);
          } else if (ch != 'M' || tp->t_disguise == 'M') {
            if (start == &hero) runto(&pos);
            if (terse)
              msg("%s misses", name);
            else
  ...
  for (c2 = spotpos; c2 < c1; c2++) mvaddch(c2->y, c2->x, chat(c2->y, c2->x));
}
