...
OPTION optlist[] = {
    ...
    {"flush", "Flush typeahead during battle", &fight_flush, put_bool,
     get_bool},
    {"jump", "Show position only at end of run", &jump, put_bool, get_bool},
    {"seefloor", "Show the lamp-illuminated floor", &see_floor, put_bool,
     get_sf},
    {"passgo", "Follow turnings in passageways", &passgo, put_bool, get_bool},
    {"tombstone", "Print out tombstone when killed", &tombstone, put_bool,
     get_bool},
    {"inven", "Inventory style", &inv_type, put_inv_t, get_inv_t},
...
void put_bool(void *b) { waddstr(hw, *(bool *)b ? "True" : "False"); }
...
void put_str(void *str) { waddstr(hw, (char *)str); }
...
void put_inv_t(void *ip) { waddstr(hw, inv_t_name[*(int *)ip]); }
...
int get_str(void *vopt, WINDOW *win) {
  ...
  for (sp = buf; (c = readchar()) != '\n' && c != '\r' && c != ESCAPE;
    ...
    else if (md_is_erasechar(c)) /* process erase character */
    {
      if (sp > buf) {
        sp--;
        for (i = (int)strlen(unctrl(*sp)); i; i--) waddch(win, '\b');
      }
      continue;
...
int get_num(void *vp, WINDOW *win) {
  ...
  if ((i = get_str(buf, win)) == NORM) *opt = (short)atoi(buf);
  return i;
}
#endif
...
void parse_opts(char *str) {
  ...
  while (*str) {
     ...
     * Get option name
     */
    for (sp = str; isalpha(*sp); sp++) continue;
    len = (int)(sp - str);
    /*
    ...
    for (op = optlist; op <= &optlist[NUM_OPTS - 1]; op++)
      if (EQSTR(str, op->o_name, len)) {
        ...
        else                           /* string option */
        {
           ...
           */
          for (sp = str + 1; *sp && *sp != ','; sp++) continue;
          /*
...
void strucpy(char *s1, char *s2, int len) {
  if (len > MAXINP) len = MAXINP;
  while (len--) {
    if (isprint(*s2) || *s2 == ' ') *s1++ = *s2;
    s2++;
  }
  *s1 = '\0';
}
