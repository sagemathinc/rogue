...
char *inv_name(THING *obj, bool drop) {
  ...
  switch (obj->o_type) {
    case POTION:
      ...
      when STICK : nameit(obj, ws_type[which], ws_made[which], &ws_info[which],
                          charge_str);
      when SCROLL : if (obj->o_count == 1) {
        strcpy(pb, "A scroll ");
        pb = &prbuf[9];
      }
      else {
      ...
      when FOOD : if (which == 1) if (obj->o_count == 1)
                      sprintf(pb, "A%s %s", vowelstr(fruit), fruit);
      else sprintf(pb, "%d %ss", obj->o_count, fruit);
      else if (obj->o_count == 1) strcpy(pb, "Some food");
      else sprintf(pb, "%d rations of food", obj->o_count);
      when WEAPON : sp = weap_info[which].oi_name;
      if (obj->o_count > 1)
      ...
      when ARMOR : sp = arm_info[which].oi_name;
      if (obj->o_flags & ISKNOW) {
        sprintf(pb, "%s %s [", num(a_class[which] - obj->o_arm, 0, ARMOR), sp);
        if (!terse) strcat(pb, "protection ");
        pb = &prbuf[strlen(prbuf)];
      ...
      when AMULET : strcpy(pb, "The Amulet of Yendor");
      when GOLD : sprintf(prbuf, "%d Gold pieces", obj->o_goldval);
#ifdef MASTER
    otherwise:
...
void drop() {
  ...
  if ((obj = get_item("drop", 0)) == NULL) return;
  ...
  obj->o_pos = hero;
  if (obj->o_type == AMULET) amulet = FALSE;
...
THING *new_thing(bool initial_object) {
  ...
  switch (no_food > 3 ? 2 : pick_one(things, NUMTHINGS)) {
    case 0:
      ...
      when 1 : cur->o_type = SCROLL;
      cur->o_which = pick_one(scr_info, MAXSCROLLS);
      when 2 : cur->o_type = FOOD;
      no_food = 0;
      if (rnd(10) != 0)
        cur->o_which = 0;
      else
        cur->o_which = 1;
      when 3 : init_weapon(cur, pick_one(weap_info, MAXWEAPONS));
      r = rnd(100);
  ...
  if (initial_object == TRUE) make_known(cur);
  return cur;
}
...
void discovered() {
  ...
  do {
    ...
    if (!terse) addmsg("for ");
    addmsg("what type");
    if (!terse) addmsg(" of object do you want a list");
    msg("? (* for all)");
    ch = readchar();
    switch (ch) {
      case ESCAPE:
      ...
      default:
        if (terse)
          ...
          msg("Please type one of %c%c%c%c (ESCAPE to quit)", POTION, SCROLL,
              RING, STICK);
    }
  } while (!disc_list);
  if (ch == '*') {
...
void print_disc(char type) {
  ...
  if (num_found == 0) add_line(nothing(type), NULL);
}
...
char add_line(char *fmt, char *arg) {
  ...
  if (line_cnt == 0) {
    wclear(hw);
    if (inv_type == INV_SLOW) mpos = 0;
  }
  if (inv_type == INV_SLOW) {
    if (*fmt != '\0')
      if (msg(fmt, arg) == ESCAPE) return ESCAPE;
    line_cnt++;
  } else {
    if (maxlen < 0) maxlen = (int)strlen(prompt);
    if (line_cnt >= LINES - 1 || fmt == NULL) {
      if (inv_type == INV_OVER && fmt == NULL && !newpage) {
        ...
        for (y = 0; y <= line_cnt; y++) {
          wmove(sw, y, 0);
          for (x = 0; x <= maxlen; x++) waddch(sw, mvwinch(hw, y, x));
        }
        wmove(tw, line_cnt, 1);
        waddstr(tw, prompt);
        /*
...
void make_known(THING *obj) {
  ...
  switch (obj->o_type) {
    case POTION:
      op = &pot_info[which];
      when RING : op = &ring_info[which];
      when STICK : op = &ws_info[which];
      when SCROLL : op = &scr_info[which];
      when WEAPON : op = &weap_info[which];
      when ARMOR : op = &arm_info[which];
    otherwise:
      break;
  }
  if (op != NULL) {
