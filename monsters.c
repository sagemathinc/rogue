...
void new_monster(THING *tp, char type, coord *cp) {
  ...
  if (level > 29) tp->t_flags |= ISHASTE;
  tp->t_turn = TRUE;
  tp->t_pack = NULL;
  if (ISWEARING(R_AGGR)) runto(cp);
  if (ISVERSION_5_3()) {
    if (type == 'M') {
    ...
    if (type == 'X') tp->t_disguise = rnd_thing();
  }
}
...
#ifdef MASTER
  if ((tp = moat(y, x)) == NULL) msg("can't find monster in wake_monster");
#else
  tp = moat(y, x);
  if (tp == NULL) endwin(), abort();
#endif
  ch = tp->t_type;
  /*
...
int save(int which) {
  if (which == VS_MAGIC) {
    if (ISRING(LEFT, R_PROTECT)) which -= cur_ring[LEFT]->o_arm;
    if (ISRING(RIGHT, R_PROTECT)) which -= cur_ring[RIGHT]->o_arm;
  }
