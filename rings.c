...
void ring_on() {
  ...
  if (!terse) addmsg("you are now wearing ");
...
int ring_eat(int hand) {
  ...
  if ((ring = cur_ring[hand]) == NULL) return 0;
  if ((eat = uses[ring->o_which]) < 0) eat = (rnd(-eat) == 0);
  if (ring->o_which == R_DIGEST) eat = -eat;
  return eat;
}
