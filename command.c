...
void command() {
  ...
  while (ntimes--) {
     ...
     */
    if (on(player, ISSLOW | ISGREED | ISINVIS | ISREGEN | ISTARGET)) exit(1);
    ...
    lastscore = purse;
    move(hero.y, hero.x);
    if (!((running || count) && jump)) refresh(); /* Draw screen */
    take = 0;
    after = TRUE;
    /*
      ...
      switch (ch) {
        case ',': {
          ...
          if (found) {
            ...
            if (!terse) addmsg("there is ");
            addmsg("nothing here");
            if (!terse) addmsg(" to pick up");
            endmsg();
          }
        }
          when '!' : shell();
          ...
          when CTRL('H')
              ...
              : case CTRL('L')
              : case CTRL('Y')
              : case CTRL('U') : case CTRL('B') : case CTRL('N') : {
        ...
        case 'f':
          ...
          when 't' : if (!get_dir()) after = FALSE;
          else missile(delta.y, delta.x);
          when 'a' : if (last_comm == '\0') {
          ...
          if (get_dir()) {
            ...
            if (!terse) addmsg("You have found ");
          ...
          when '=' : current(cur_ring[LEFT], "wearing",
...
void help() {
  ...
  numprint = 0;
  for (strp = helpstr; strp->h_desc != NULL; strp++)
    if (strp->h_print) numprint++;
  if (numprint & 01) /* round odd numbers up */
    numprint++;
  numprint /= 2;
  if (numprint > LINES - 1) numprint = LINES - 1;
  ...
  for (strp = helpstr; strp->h_desc != NULL; strp++)
    if (strp->h_print) {
      ...
      if (strp->h_ch) waddstr(hw, unctrl(strp->h_ch));
      waddstr(hw, strp->h_desc);
      if (++cnt >= numprint * 2) break;
    }
  wmove(hw, LINES - 1, 0);
...
void identify() {
  ...
  register char *str;
  static struct h_list ident_list[] = {{'|', "wall of a room", FALSE},
 ...
 */
bool levit_check() {
  if (!on(player, ISLEVIT)) return FALSE;
...
void call() {
  ...
  switch (obj->o_type) {
    case RING:
      ...
      goto norm;
      when POTION : op = &pot_info[obj->o_which];
      elsewise = p_colors[obj->o_which];
      goto norm;
      when SCROLL : op = &scr_info[obj->o_which];
      elsewise = s_names[obj->o_which];
      goto norm;
      when STICK : op = &ws_info[obj->o_which];
      elsewise = ws_made[obj->o_which];
    norm:
      know = &op->oi_know;
      guess = &op->oi_guess;
      if (*guess != NULL) elsewise = *guess;
      when FOOD : msg("you can't call that anything");
      return;
    otherwise:
      guess = &obj->o_label;
      know = NULL;
