...
int main(int argc, char **argv, char **envp) {
  ...
  while (argc >= 2) {
    ...
    if (strcmp(argv[1], "-w") == 0) {
      if (strcmp(PASSWD, md_crypt(md_getpass("wizard's password: "), "mT")) ==
          0) {
        wizard = TRUE;
  ...
  if (argc == 2)
    if (!restore(argv[1], envp)) /* Note: restore will never return */
      my_exit(1);
  printf("Hello %s %s, just a moment while I dig the dungeon...",
         (rookie_mode ? "rookie" : "warrior"), whoami);
#ifdef MASTER
  if (wizard) printf(" #%d", dnum);
#endif
  fflush(stdout);
...
void playit() {
  ...
  if (md_hasclreol()) inv_type = INV_CLEAR;

  /*
   * parse environment declaration of options
   */
  if ((opts = getenv("ROGUEOPTS")) != NULL) parse_opts(opts);
